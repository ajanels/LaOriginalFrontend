...
import { FormasPagoService, FormaPagoItem, FormaPagoCreate } from '../../../services/formas-pago.service';
...
export class FormasPago implements OnInit, OnDestroy {
  ...
  cargar(): void {
    ...
    this.svc.list(false).subscribe({
      next: (data: FormaPagoItem[]) => { this.formas = data ?? []; this.aplicarFiltros(); this.cargando = false; },
...
async eliminarForma(f: FormaPagoItem): Promise<void> {
  ...
  guardar(): void {
    ...
    const dto: FormaPagoCreate = {
      ...
      afectaCaja: !!this.nuevo.afectaCaja,
      afectaBanco: !!this.nuevo.afectaBanco,
    ...
    if (this.editando) {
      this.svc.update(this.editando.id, dto).subscribe({
        next: () => {
          ...
          this.aplicarFiltros(); this.Toast.fire({icon:'success', title:'Forma de pago actualizada'});
          this.cargando = false; this.cerrarModal();
        },
        error: (err) => {
    ...
    this.svc.create(dto).subscribe({
      next: (f) => {
        this.formas.unshift(f); this.aplicarFiltros();
  ...
  async confirmarToggle(f: FormaPagoItem): Promise<void> {
    ...
    const res = await Swal.fire({ title: activar ? '¿Activar forma de pago?' : '¿Desactivar forma de pago?',
      text: `"${f.nombre}" cambiará de estado.`, icon:'question', showCancelButton:true,
